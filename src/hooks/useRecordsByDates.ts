...
export const useRecordsByDates = (dataFrom: string, dataTo:string) => {
    const {isLoading, data:records} = useQuery(
        [QueryKeys.GetNOEListByDates, dataFrom, dataTo],
        () => ObjectsService.getObjectsByDates(dataFrom, dataTo),
        {
            select: ({data}) => {
                let result: any[] = []
